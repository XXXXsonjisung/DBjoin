...
FROM DEPARTMENT 
JOIN LOCATION
/* INNER */ ON (LOCATION_ID = LOCAL_CODE);

-- 1) LEFR[OUTER] JOIN : 합치기에 사용한 두 테이블중 왼편에 기술된 테이블의 
-- 컬럼 수를 기준으로 JOIN
...
ON (DEPT_CODE = DEPT_ID); -- 24행 (마케팅부, 국내영업부, 해외영업 3부 포함)
...
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE(+) = DEPT_ID(+);
-- 오라클은 풀아웃터 조인 안됌 - 아우터 조인된 테이블을 1개만 지정 가능
...
FROM EMPLOYEE
JOIN DEPARTMENT DON(DEPT_CODE = DOPT_ID)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE);
...
JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE)
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID);
--Error position: line: 251 pos: 40
...
-- 사원명, 주민번호, 부서명, 직급명을 조회하시오.(사원명 EMP_NAME,주민번호 EMP_NO,직급명 JOB,부서명 DEPT_TITLE,)\
-- EMPLOYEE 주민 번호 , 사원명 / JOB 직급명 / DEPARTMENT 부서명
...
--사번EMP_ID,직원명EMP_NAME(EMPLOYEE)/직급명JOB_NAME(JOB)/부서명DEPT_TITLE(DEPARTMENT)
...
-- 사원명EMP_NAME(EMPLOYEE)/직급명JOB_NAME(JOB)/부서명DEPT_TITLE(DEPARTMENT)
...
--사원명EMP_NAME,보너스율BONUS(EMPLOYEE)/부서명DEPT_TITLE(DEPARTMENT)/지역명LOCAL_NAME(LOCATION)
...
--사원명EMP_NAME(EMPLOYEE)/직급명JOB_NAME(JOB)/부서명DEPT_TITLE(DEPARTMENT)/지역명LOCAL_NAME(LOCATION)
...
--사원명EMP_NAME,급여SALARY(EMPLOYEE)/직급명JOB_NAME(JOB)
